<script lang="ts">
	...
	import { ArrowLeft, Save, Youtube, Link as LinkIcon, MonitorPlay, Trash2 } from 'lucide-svelte';
	import ConfirmDialog from '$lib/components/ConfirmDialog.svelte';
	import { t } from '$lib/i18n/index.js';

	let { data, form }: { data: any, form?: { message?: string; details?: string; success?: boolean } } = $props();
...
<svelte:head>
	<title>{t('admin.videos.editVideo', { defaultValue: 'Edit Video' })} - Admin</title>
</svelte:head>
	...
	<div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between rounded-3xl bg-surface p-6 sm:p-8 shadow-sm border border-border-subtle">
		<div>
			<a href="/admin/videos" class="inline-flex items-center text-xs sm:text-sm font-semibold text-text-muted hover:text-primary transition-colors mb-3 group">
				...
				{t('admin.videos.backToList', { defaultValue: 'Back to list' })}
			</a>
			<h1 class="text-2xl sm:text-3xl font-extrabold text-text-main tracking-tight flex items-center gap-3">
				...
				<span>{t('admin.videos.editVideo', { defaultValue: 'Edit Video' })}</span>
			</h1>
		</div>
	</div>
			...
			<div class="rounded-3xl bg-surface shadow-sm border border-border-subtle p-6 sm:p-8 space-y-5">
				<label class="block">
					<span class="block text-xs sm:text-sm font-bold text-text-main mb-2 flex items-center gap-2">
						<Youtube strokeWidth={1.75} size={16} class="text-rose-500" /> {t('admin.videos.videoTitle', { defaultValue: 'Video Title' })} <span class="text-red-500">*</span>
					</span>
					<input
						type="text"
						name="title"
						...
						placeholder={t('admin.videos.videoTitlePlaceholder', { defaultValue: 'e.g., SvelteKit Tutorial 2026' })}
					/>
				</label>

				<label class="block">
					<span class="block text-xs sm:text-sm font-bold text-text-main mb-2 flex items-center gap-2">
						<LinkIcon size={16} class="text-primary" /> {t('admin.videos.videoUrl', { defaultValue: 'Video URL' })} <span class="text-red-500">*</span>
					</span>
					<input
						type="url"
						name="url"
		...
		<div class="space-y-6">
			...
			<div class="rounded-3xl bg-surface shadow-sm border border-border-subtle p-5 space-y-3">
				...
				>
					...
					<span>{isSubmitting ? t('admin.videos.saving', { defaultValue: 'Saving...' }) : t('admin.videos.saveChanges', { defaultValue: 'Save Changes' })}</span>
				</button>

				<button
					type="button"
					onclick={() => showDeleteConfirm = true}
					class="w-full flex items-center justify-center gap-2 rounded-2xl bg-red-500/10 hover:bg-red-500/20 text-red-600 dark:text-red-400 py-2.5 text-xs font-bold transition-all cursor-pointer"
				>
					<Trash2 strokeWidth={1.75} size={16} />
					<span>{t('admin.videos.deleteVideo', { defaultValue: 'Delete this Video' })}</span>
				</button>
			</div>
			...
			<div class="rounded-3xl bg-surface shadow-sm border border-border-subtle p-5 space-y-3">
				<label class="block">
					<span class="block text-xs sm:text-sm font-bold text-text-main mb-2 flex items-center gap-2">
						<MonitorPlay strokeWidth={1.75} size={16} /> {t('admin.videos.platform', { defaultValue: 'Platform' })} <span class="text-red-500">*</span>
					</span>
					<select
						name="platform"
						required
					...
					>
						<option value="YOUTUBE" selected={data.video.platform === 'YOUTUBE' || data.video.platform === 'YouTube'}>YouTube</option>
						<option value="TIKTOK" selected={data.video.platform === 'TIKTOK' || data.video.platform === 'TikTok'}>TikTok</option>
						<option value="VIMEO" selected={data.video.platform === 'VIMEO' || data.video.platform === 'Vimeo'}>Vimeo</option>
						<option value="FACEBOOK" selected={data.video.platform === 'FACEBOOK' || data.video.platform === 'Facebook'}>Facebook</option>
						<option value="OTHER" selected={data.video.platform === 'OTHER' || data.video.platform === 'Other'}>{t('admin.videos.platformOther', { defaultValue: 'Other' })}</option>
					</select>
				</label>
			</div>
		</div>
...
<ConfirmDialog
	...
	message={t('admin.dialog.deleteVideoConfirm', { title: data.video?.title || '', defaultValue: `Are you sure you want to delete video "${data.video?.title || ''}"? This action cannot be undone.` })}
	loading={isDeleting}
	onconfirm={() => {
