<script lang="ts">
	...
	import { ArrowLeft, Save, FolderGit2, Link as LinkIcon, Tag, AlignLeft, Trash2 } from 'lucide-svelte';
...
<svelte:head>
	<title>{t('admin.projects.editProject', { defaultValue: 'Edit Project' })} - Admin</title>
</svelte:head>
	...
	<div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between rounded-3xl bg-surface p-6 sm:p-8 shadow-sm border border-border-subtle">
		<div>
			...
			<h1 class="text-2xl sm:text-3xl font-extrabold text-text-main tracking-tight flex items-center gap-3">
				...
				<span>{t('admin.projects.editProject', { defaultValue: 'Edit Project' })}</span>
			</h1>
		</div>
	</div>
	...
	>
		...
		<div class="space-y-6">
			<div class="rounded-3xl bg-surface shadow-sm border border-border-subtle p-6 sm:p-8 space-y-5">
				...
				<label class="block">
					<span class="block text-xs sm:text-sm font-bold text-text-main mb-2 flex items-center gap-2">
						<AlignLeft strokeWidth={1.75} size={16} class="text-indigo-500" /> {t('admin.projects.projectDescription', { defaultValue: 'Project Description' })} <span class="text-red-500">*</span>
					</span>
					<textarea
						name="description"
						rows="4"
						...
						placeholder={t('admin.projects.projectDescriptionPlaceholder', { defaultValue: 'Brief description of goals and outcomes...' })}
		...
		<div class="space-y-6">
			...
			<div class="rounded-3xl bg-surface shadow-sm border border-border-subtle p-5 space-y-3">
				...
				>
					...
					<span>{isSubmitting ? t('admin.projects.saving', { defaultValue: 'Saving...' }) : t('admin.projects.saveChanges', { defaultValue: 'Save Changes' })}</span>
				</button>
				...
				>
					...
					<span>{t('admin.projects.deleteProject', { defaultValue: 'Delete this Project' })}</span>
				</button>
			</div>
			...
			<div class="rounded-3xl bg-surface shadow-sm border border-border-subtle p-5 space-y-4">
				...
				<label class="block">
					...
					<input
						type="text"
						name="tags"
						value={Array.isArray(data.project.tags) ? data.project.tags.join(', ') : (data.project.tags || '')}
						class="w-full rounded-xl bg-soft-bg border border-border-subtle px-3 py-2 text-text-main focus:outline-none focus:border-primary text-xs sm:text-sm"
...
<ConfirmDialog
	...
	message={t('admin.dialog.deleteProjectConfirm', { title: data.project?.title || '', defaultValue: `Are you sure you want to delete project "${data.project?.title || ''}"? This action cannot be undone.` })}
	loading={isDeleting}
	onconfirm={() => {
